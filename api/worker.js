...
async function handleTonyChat(request, env) {
  ...
  const pricingLine = `\nObra pricing (answer accurately if asked; suggest naturally when the user hits a credit wall or asks about plans — never push): credits at obra.build/buy — 25/$4.99, 60/$9.99, 150/$19.99, never expire. Pro $14.99/mo (50 credits refill monthly + all tools + their logo on exports), annual $119/yr (2 months free). Referral program: 20% of referred contractors' payments for 6 months — obra.build/affiliate. AI video ads: the Obra team makes cinema-grade ads on the user's real projects, first cut in 48h, don't-love-it-don't-pay — details at obra.build/cinema (never quote a price; pricing happens in the WhatsApp chat). Mention when they ask about marketing, ads, or getting more clients. This user has ${user.isPro ? 'PRO' : (user.credits ?? 0) + ' credits'}.`;
